...
init_mnesia_tables(State) ->
	...
	case ejabberd_config:get_local_option({store_group_members, Domain},
                                       fun(V) -> V end) of
		1 ->
	...
	%% 消息表，备份节点添加
	MsgCopyNodes = case ejabberd_config:get_local_option({ram_msg_bak_nodes, Domain},
                                       fun(V) -> V end) of
					   undefined ->
						   [];
					   N ->
...
%% 	mnesia:add_table_index(RamMsgTableName, to),	
	...
	%% 用户数据存储表
	case ejabberd_config:get_local_option({store_user_tables_info, Domain},
                                       fun(V) -> V end) of
		1 ->
...
init_msyql_conn() ->
	[Domain|_] = ?MYHOSTS,
	case ejabberd_config:get_local_option({mysql_config, Domain},
                                       fun(V) -> V end) of
		undefined ->
...
%% 			?ERROR_MSG("dbcfg ~p", [DBCfg]),
			...
			emysql:add_pool(?DB, ?DBCONNNUM, binary_to_list(User), 
...
get_data_node(Domain) ->
	[DataNode] 
		= case ejabberd_config:get_local_option({data_nodes, Domain},
												fun(V) -> V end) of
			  undefined ->
				  [];
			  N ->
